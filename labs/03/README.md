# Лабораторная работа №3 — Защита данных с помощью мьютексов

- [Лабораторная работа №3 — Защита данных с помощью мьютексов](#лабораторная-работа-3--защита-данных-с-помощью-мьютексов)
  - [Задания](#задания)
    ...
    - [Задание №3 — BoundedBlockingQueue — 50 баллов](#задание-3--boundedblockingqueue--50-баллов)
      - [Требования к классу](#требования-к-классу)
      - [Поведение](#поведение)
        - [1. Ограниченная ёмкость](#1-ограниченная-ёмкость)
        - [2. Блокирующее извлечение](#2-блокирующее-извлечение)
        - [3. Закрытие очереди](#3-закрытие-очереди)
        - [4. Корректность](#4-корректность)
      - [Приложение-демонстрация](#приложение-демонстрация)
        - [Поведение:](#поведение-1)
        - [Инварианты](#инварианты)
      - [Гарантии безопасности](#гарантии-безопасности)
      - [Бонус за операции с таймаутом — +20 баллов](#бонус-за-операции-с-таймаутом--20-баллов)
...
### Задание №3 — BoundedBlockingQueue — 50 баллов

Разработать потокобезопасную ограниченную блокирующую очередь `BoundedBlockingQueue<T>`
с поддержкой корректного завершения работы потоков.

Задание направлено на отработку:

- `std::mutex`
- `std::condition_variable`
- работы с несколькими условиями ожидания
- корректного завершения потоков
- отсутствия busy-wait
- базовой гарантии безопасности при исключениях
...
public:
    ...
    // Выбрасывает std::logic_error, если очередь закрыта.
    void Push(T value);

    // Пытается добавить элемент.
    // Возвращает false, если очередь переполнена или закрыта.
    ...
    // После закрытия:
    //  - Push запрещён
    //  - ожидающие потоки должны быть разбужены
    void Close();
...
- В очередь нельзя добавить больше элементов, чем `capacity`.
- `Push()` должен блокироваться до появления свободного места.
- `TryPush()` не блокируется.

##### 2. Блокирующее извлечение

- `Pop()` блокируется, если очередь пуста.
- Если очередь закрыта и пуста — возвращает `false`.

##### 3. Закрытие очереди

Метод `Close()`:

- запрещает добавление новых элементов;
- будит все ожидающие потоки;
- гарантирует, что потоки не зависнут навсегда.

##### 4. Корректность

- Не должно быть busy-wait.
- Не должно быть дедлоков.
- Не должно быть утечек данных.
- Все операции потокобезопасны.
- **Должны быть разработаны тесты** для проверки работы очереди в конкурентной среде.
...
```bash
bbqueue NUM_PRODUCERS NUM_CONSUMERS CAPACITY
```
...
- Потоки-производители:
  ...
  - периодически генерируют числа;
  - добавляют их в очередь;
  - ведут счётчик произведённых элементов.

- Потоки-потребители:

  - извлекают элементы;
  - ведут счётчик потреблённых элементов.

- Главный поток:

  - запускает симуляцию на заданное время (например, 10 секунд);
  - либо завершает работу по сигналу SIGINT/SIGTERM;
  - вызывает `Close()` очереди;
...
- Общее количество произведённых элементов должно быть равно общему количеству потреблённых элементов.
- Очередь не должна содержать элементов после завершения.

#### Гарантии безопасности

Класс должен:

- предоставлять не ниже базовой гарантии безопасности при исключениях;
- не допускать повреждения внутреннего состояния;
- корректно работать при выбросе исключений из конструктора/перемещения `T`.

#### Бонус за операции с таймаутом — +20 баллов

1. Реализовать `PopFor(std::chrono::duration)` — извлечение с тайм-аутом.
2. Реализовать `PushFor(...)`.
3. Добавить статистику:

   - количество блокировок;
   - максимальный размер очереди.
4. Использовать `std::stop_token` для кооперативной остановки потоков.
