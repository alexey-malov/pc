...
- [Вопросы к экзамену/зачёту](#вопросы-к-экзаменузачёту)
  ...
  - [Иерархии памяти](#иерархии-памяти)
  - [Модель памяти и атомарные операции](#модель-памяти-и-атомарные-операции)
  - [Протоколы когерентности кешей](#протоколы-когерентности-кешей)
  - [Многопоточная модель памяти C++](#многопоточная-модель-памяти-c)
  - [Lock-free структуры данных - 1](#lock-free-структуры-данных---1)
  - [Lock-free структуры данных - 2](#lock-free-структуры-данных---2)
  - [Корутины](#корутины)
  - [Гетерогенные вычисления с помощью OpenCL](#гетерогенные-вычисления-с-помощью-opencl)
...
- Почему современные процессоры используют многоуровневую иерархию памяти вместо единого большого и быстрого хранилища данных?
- Что такое cache hit и cache miss? Как они влияют на производительность программы?
- Что такое пространственная локальность данных? Приведите примеры
- Что такое временная локальность данных? Приведите примеры.
- Приведите пример алгоритма с хорошей временной локальностью и пример алгоритма с плохой временной локальностью.
- Почему последовательный обход массива обычно работает быстрее случайного доступа к его элементам?
- Почему последовательный обход большого массива может работать почти так же быстро, как обход небольшого массива,
  помещающегося в кеш?
- Какие проблемы, связанные с памятью, могут ограничивать масштабируемость параллельной программы?
- Что такое false sharing и почему он возникает?
- Какие признаки указывают на то, что производительность ограничена скоростью доступа к памяти, а не вычислениями?

## Модель памяти и атомарные операции

- Что такое модель памяти в языке C++ и зачем она нужна при многопоточном программировании?
- Почему до C++11 написание потокобезопасного кода зависело от компилятора и платформы?
- Что такое «порядок модификаций объекта» и какую роль он играет в многопоточности?
- Что происходит, если два потока по-разному видят порядок модификаций одного и того же объекта?
- Чем отличаются атомарные и неатомарные операции в контексте модели памяти?
- Объясните идею ленивой инициализации. В чём её основное преимущество?
- Почему обычная ленивая инициализация может быть небезопасна в многопоточном коде?
- Как работает паттерн double-checked locking (DCL)? Какие проблемы могут возникнуть при его использовании?
- Как `std::call_once` обеспечивает безопасную однократную инициализацию?
- Что произойдет, если вызываемая через `call_once` функция выбросит исключение?
- В чём особенность типа `std::atomic_flag` по сравнению с другими атомарными типами?
- Какие операции предоставляет `std::atomic` и что делает каждая из них (например, `store`, `load`, `exchange`, `compare_exchange_weak`)?
- Что делает метод `is_lock_free`, и зачем нужен `is_always_lock_free`?
- Как переупорядочивание операций компилятором и процессором может повлиять на корректность многопоточной программы?
- Какие ограничения накладываются на спекулятивное выполнение в модели памяти C++?

## Протоколы когерентности кешей

- Что такое когерентность кеша и зачем она нужна в многопроцессорных системах?  
- Объясните разрыв производительности между процессорами и памятью. Как кэши помогают его компенсировать?  
- Перечислите и сравните три политики размещения данных в кэше:
  прямое отображение, полностью ассоциативное и набор-ассоциативное.
  Охарактеризуйте преимущества и недостатки каждой политики, а также их влияние на производительность.
- Какие состояния кэш-линий описывает протокол MESI? Что означает каждое из них?
  Дайте краткое описание состояний и их роль в обеспечении когерентности.*
- Опишите, как работает переход кэш-линии из состояния Modified в Exclusive (M → E).
  Требуется ли отправка сообщений?  
- Что такое промах кэша (cache miss)? Перечислите типы промахов и объясните их причины.
- Почему использование буферов записи (store buffer) необходимо в протоколах когерентности?
  Приведите пример проблемы, которую они решают.
- Что такое нарушение глобального порядка памяти? Приведите пример кода, демонстрирующий эту проблему.
- Какие типы барьеров памяти существуют? В чём разница между полным барьером (smp_mb), барьером чтения и барьером записи?
- Что такое очередь инвалидации (invalidate queue)? Как она помогает минимизировать задержки в протоколе MESI?
- Почему в MESI-протоколе процессор должен подождать подтверждения инвалидации перед выполнением записи? Приведите пример.
- Какие сообщения используются в протоколе MESI для взаимодействия между процессорами? Объясните их назначение.
- Что такое store forwarding и как он может привести к нарушению ожидаемого поведения программы?
- Как барьер памяти `smp_mb()` решает проблему нарушения порядка памяти? Объясните на примере из слайдов.

## Многопоточная модель памяти C++

- Какие основные модели памяти предлагает стандарт C++11 для многопоточного программирования? В чём их отличие?
- Почему модель последовательной согласованности (Sequential Consistency) является интуитивно понятной, но дорогой?
  Приведите пример её использования.
- Какой гарантии предоставляет `memory_order_relaxed`? В каких случаях его можно применять?
- Что такое acquire-семантика и как она обеспечивает видимость данных между потоками?
- Какие роли играют `memory_order_release` и `memory_order_acquire` в передаче данных между потоками?
- Что такое memory barrier и как он влияет на порядок выполнения инструкций? Какие бывают барьеры
- Какие цели преследует использование `std::atomic_thread_fence`?
- Какие условия должны выполняться для синхронизации между `release-fence` и `acquire`-операцией?
- В чём разница между `memory_order_acq_rel` и `memory_order_seq_cst`?
- Почему `memory_order_seq_cst` требует глобальной синхронизации? Как это влияет на производительность?
- Как работают фенсы `acquire` и `release` в контексте синхронизации между потоками?
- Что такое release-последовательность и как она используется в многопоточном программировании?
- Какие ошибки могут возникнуть при неправильном использовании `memory_order_relaxed`?
- Какие компромиссы между моделью памяти и производительностью существуют в C++?

## Lock-free структуры данных - 1

- Что такое блокирующие (blocking) алгоритмы и структуры данных?
- В чём разница между lock-free, obstruction-free и wait-free алгоритмами? Приведите примеры для каждого уровня.
- Что такое спин-блокировка (spin lock)? Почему она считается неблокирующей, но не lock-free?
- Почему lock-free структуры данных важны для предотвращения deadlocks и live locks?
- Опишите принципы реализации lock-free стека.
- Какие проблемы возникают с освобождением памяти в lock-free структурах данных?
- Как реализуется отложенное удаление узлов в lock-free стеке?
- Что такое hazard pointers и как они помогают в управлении памятью?  
- В чём состоит проблема "голодания" (starvation) в lock-free алгоритмах? Приведите пример.  

## Lock-free структуры данных - 2

- Объясните, как реализуется Hazard Pointer
- Почему сканирование hazard pointer-ов при каждом `Pop` неэффективно? Какие оптимизации предлагаются?
- Как reference counting (подсчёт ссылок) с использованием `std::shared_ptr` упрощает реализацию lock-free стека?
- Какие ограничения имеет использование `atomic<shared_ptr>` для lock-free структур?
- Что такое "quiescent point" и как он используется в lock-free алгоритмах?
- Как hazard pointers сравниваются с reference counting в управлении памятью?
- Как lock-free структуры данных обеспечивают устойчивость к сбоям потоков?
- В чём недостатки lock-free и wait-free алгоритмов по сравнению с блокирующими?
- Как lock-free стек предотвращает удаление узлов, на которые ссылаются hazard pointers?
...
- В чём заключается основное различие между CPU и GPU при выполнении вычислений?
  Объясните, какие задачи лучше подходят для CPU, а какие — для GPU, и почему.
- Что такое гетерогенная платформа? Для чего применяются гетерогенные платформы?
  Приведите примеры устройств, входящих в такую платформу.
- Перечислите основные API для гетерогенных вычислений. В чём отличие OpenCL от CUDA?
- Какие компоненты в OpenCL отвечают за взаимодействие хоста и устройства?
  Опишите роль контекста, очереди команд, буферов и ядер (kernel) в процессе вычислений.
- В чём заключается модель памяти OpenCL? Какие типы памяти существуют и их особенности?
  Опишите различия между глобальной, локальной и приватной памятью и их применение.
- Как организуется параллельное выполнение задач в OpenCL с использованием NDRange и Work Groups?
- Как реализуется ядро для сложения векторов в OpenCL?
  Объясните, как `get_global_id(0)` используется для присвоения уникального ID каждому потоку.
- Почему тайловое умножение матриц более эффективно, чем классическое?
  Какой алгоритм используется в тайловом умножении матриц?
- Как создаётся out-of-order очередь команд в OpenCL и в чём её преимущество?
- Как события (cl::Event) используются для управления зависимостями между командами?
- Как выполняется профилирование времени выполнения ядра в OpenCL?
- Как передаются аргументы ядру OpenCL из основной программы?
- Какой передать результаты вычислений с устройства на хост?
- Как OpenCL позволяет перекрывать копирование данных и вычисления?
